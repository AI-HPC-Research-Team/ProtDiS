...
class BindingSite(Dataset):
    def __init__(
        ...
        with open(avro_path, "rb") as fo:
            ...
            for protein_res in tqdm(avro_reader, desc="process protein data"):
                ...
                if lazy:
                    self.data.append((pid, (esm_file, k_file, resolution, tsv_file, residue_info)))
                else:
                    fused = fuse_data(esm_file, k_file, resolution, tsv_file, residue_info)
    ...
    def __getitem__(self, idx):
        if self.lazy:
            pid, (esm_p, k_p, resolution, tsv_file, residue_info) = self.data[idx]
            sample = fuse_data(esm_p, k_p, resolution, tsv_file, residue_info)
        else:
