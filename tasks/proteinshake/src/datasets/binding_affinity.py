...
class BindingAffinity(Dataset):
    def __init__(
        self,
        root: str = './data/ligand_binding/',
        ...
        avro_path = os.path.join(root, "ProteinLigandInterfaceDataset.residue.avro")
        with open(avro_path, "rb") as fo:
            ...
            for protein_res in tqdm(avro_reader, desc="process protein data"):
                ...
                if lazy:
                    self.data.append((pid, (esm_file, k_file, resolution, tsv_file), (protein_info['neglog_aff'], protein_info['fp_morgan_r2'])))
                else:
                    fused = fuse_data(esm_file, k_file, resolution, tsv_file)
                    fused['affinity'] = protein_info['neglog_aff']  # dissociation constant
                    fused['mol_fp'] = protein_info['fp_maccs'] + protein_info['fp_morgan_r2']  # molecular fingerprints
    ...
    def __getitem__(self, idx):
        if self.lazy:
            pid, (esm_file, k_file, resolution, tsv_file), (affinity, mol_fp) = self.data[idx]
            sample = fuse_data(esm_file, k_file, resolution, tsv_file)
            sample['affinity'] = affinity
            sample['mol_fp'] = mol_fp
        else:
        ...
        y = torch.tensor(sample['affinity'], dtype=torch.float)
        ...
        data = Data(
            ...
            edge_attr=None,
            y=y,
            other=torch.tensor(sample['mol_fp'], dtype=torch.float).unsqueeze(0),  # [1, num_features] for molecular fingerprints
        )
        return data
...
if __name__ == "__main__":
    ...
    for data in data_loader:
        assert len(data.seq) == len(data.y)
