...
class EnzymeClass(Dataset):
    def __init__(
        self,
        root: str = './data/enzyme_class/',
        ...
        avro_path = os.path.join(root, "EnzymeCommissionDataset.residue.avro")
        with open(avro_path, "rb") as fo:
            ...
            for protein_res in tqdm(avro_reader, desc="process protein data"):
                ...
                raw_labels.append(protein_info['EC'].split(".")[ec_level])

                if max_samples and len(self.data) >= max_samples: break

        # map string labels to integers
        #self.label_map = {label: i for i, label in enumerate(sorted(set(raw_labels)))}
        self.label_map = {'1': 0, '2': 1, '3': 2, '4': 3, '5': 4, '6': 5, '7': 6}
        self.labels = [self.label_map[lab] for lab in raw_labels]
...
if __name__ == "__main__":
    ...
    for data in data_loader:
        ...
        assert len(data.seq) == len(data.lp)
        assert len(data.seq) == len(data.hydro)
        assert len(data.seq) == len(data.stability)
        assert len(data.seq) == len(data.entropy)
        assert len(data.seq) == len(data.ss_label)
        assert len(data.seq) == len(data.asa_label)
        assert len(data.seq) == len(data.bfactor_label)
        assert len(data.seq) == len(data.lp_label)
        assert len(data.seq) == len(data.hydro_label)
        assert len(data.seq) == len(data.stability_label)
        assert len(data.seq) == len(data.entropy_label)
        ...
        print(data.stability_label)
        print(data.entropy_label)
        break
