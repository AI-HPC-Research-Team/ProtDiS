...
class StructuralClass(Dataset):
    def __init__(
        ...
        assert scop_level in ['tp', 'cl', 'cf', 'sf', 'fa'], f"Invalid scop_level: {scop_level}. Must be one of ['tp', 'cl', 'cf', 'sf', 'fa']"
        ...
        self.scop_level = {'tp': 'SCOP-TP',
                          'cl': 'SCOP-CL',
                          'cf': 'SCOP-CF',
                          'sf': 'SCOP-SF',
                          'fa': 'SCOP-FA'}[scop_level]
        ...
        avro_path = os.path.join(root, "SCOPDataset.residue.avro")
        with open(avro_path, "rb") as fo:
            ...
            for protein_res in tqdm(avro_reader, desc="process protein data"):
                ...
                label_set.add(protein_info[self.scop_level])
                ...
                # load esm_encodings
                pid = protein_info['ID']
                esm_files = glob.glob(os.path.join(root, "esm_encodings", f"{pid}*.npz"))
                if len(esm_files) != 1:
                ...
                if lazy:
                    self.data.append((pid, (esm_files[0], esm_files[0].replace("esm_encodings", "k_encodings"), resolution, tsv_file)))
                else:
                    fused = fuse_data(esm_files[0], esm_files[0].replace("esm_encodings", "k_encodings"), resolution, tsv_file)
                    self.data.append((pid, fused))
                raw_labels.append(protein_info[self.scop_level])

        if max_samples:
            idxs = random.sample(range(len(self.data)), max_samples)
            self.data = [self.data[i] for i in idxs]
            raw_labels = [raw_labels[i] for i in idxs]
        ...
        logging.info(f"Scop level={self.scop_level}, split={split_type}, redundancy={redundancy}, resolution={resolution}: {len(self.data)} proteins in '{split}' set with {self.num_classes} classes.")
